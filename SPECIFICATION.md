...
|------|-----------|
| HTTP-роутер | chi v5 |
| База данных | PostgreSQL via pgx v5 (pgxpool) |
| Миграции | goose v3, встроены через `embed.FS` |
...
```
cmd/gophermart/              — точка входа, сборка зависимостей, запуск сервера
internal/config/             — флаги CLI и переменные окружения
internal/domain/             — модели (User, Order, Balance, Withdrawal) и ошибки-сентинели
internal/luhn/               — валидация номера заказа по алгоритму Луна
internal/repository/         — интерфейсы репозиториев
internal/repository/postgres/— реализация репозиториев на PostgreSQL
internal/service/            — бизнес-логика (auth, order, balance, withdrawal)
internal/handler/            — HTTP-хендлеры и chi-роутер
internal/middleware/         — JWT-аутентификация, gzip, логирование запросов
internal/accrual/            — HTTP-клиент и фоновый поллер внешнего сервиса начислений
internal/admin/              — как бонус - административная панель (отдельный роутер /admin/)
migrations/                  — SQL-файлы goose, встроены через embed.FS
web/                         — как бонус - статика Bootstrap SPA для админки, встроена через embed.FS
```

### Баланс

Баланс не хранится отдельной колонкой — вычисляется запросом на лету, чтобы исключить рассинхронизацию:

```sql
current = SUM(orders.accrual WHERE status='PROCESSED') - SUM(withdrawals.sum)
```

### Атомарное списание

Списание (`POST /api/user/balance/withdraw`) выполняется одним CTE-запросом: проверка баланса и вставка строки происходят в рамках одного обращения к БД. Если баланса не хватает — запрос не вставляет строку, сервис возвращает `402`.

### Алгоритм Луна

Номер заказа проверяется по алгоритму Луна — той же контрольной сумме, что используется в номерах банковских карт. Это требование задания. Проверка отсеивает опечатки и случайные числа ещё до обращения к базе данных.

### Аутентификация

JWT выдаётся при регистрации и логине, передаётся одновременно в заголовке `Authorization: Bearer` и cookie `token; HttpOnly`. Middleware проверяет заголовок первым, затем cookie. TTL — 24 часа.

### Фоновый поллер начислений

Горутина `accrual.Poller` раз в секунду запрашивает статус незавершённых заказов. Параллельность ограничена семафором (3 воркера). При ответе `429 Too Many Requests` читается заголовок `Retry-After` и поллер делает паузу на указанное время.

### Логирование

Каждый запрос логируется middleware: метод, путь, статус, длительность, user_id. Хендлеры могут обогащать лог-строку дополнительными полями через `middleware.AddLogFields` (например, логин пользователя или номер заказа).

### Административная панель

Bootstrap SPA по адресу `/admin/`. Использует отдельный JWT с полем `role: admin` — токены обычных пользователей не принимаются. Показывает статистику, список пользователей с балансами, все заказы и списания.
