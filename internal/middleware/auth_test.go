...
type mockValidator struct {
	validateFn func(token string) (int64, error)
}

func (m *mockValidator) ValidateToken(token string) (int64, error) {
	return m.validateFn(token)
}

// nextHandler is a simple handler that captures the user ID from context.
func nextHandler(t *testing.T, expectedUserID int64) http.Handler {
...
func TestAuthMiddleware_ValidCookieToken(t *testing.T) {
	validator := &mockValidator{
		validateFn: func(token string) (int64, error) { return 7, nil },
	}
...
func TestAuthMiddleware_InvalidToken(t *testing.T) {
	validator := &mockValidator{
		validateFn: func(_ string) (int64, error) { return 0, fmt.Errorf("bad token") },
	}
	handler := middleware.Auth(validator)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Error("handler should not be called")
	}))
...
func TestWithUserID(t *testing.T) {
	...
	assert.Equal(t, int64(99), middleware.UserIDFromCtx(ctx))
}

// TestUserIDFromCtx_Missing verifies 0 is returned when no user ID is in context.
func TestUserIDFromCtx_Missing(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	assert.Equal(t, int64(0), middleware.UserIDFromCtx(req.Context()))
}
